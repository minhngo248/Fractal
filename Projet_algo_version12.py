...
def traitementTest_(name):
    ...
    if  countn >= 6  and len(axiome)==1: # On a un maximum de 2 règles dans le cadre du projet, donc on a entre 6 et 7 éléments.
        # print("Le fichier consigne a le bon format") #On a le bon nombre d'éléments et ceux-ci sont marqués correctement.
        return True
...
def ecriturefichier_(tupleinput,position):
    axiome = tupleinput[0]
    newaxiome = 'kplm'+axiome+'o'
    angle = tupleinput[2]
    taille = tupleinput[3]
    fichier = open("resultat.py","w")   
    dico = {"a":"pd();forward("+str(taille)+");", "b":"pu();forward("+str(taille)+");",
            "-":"left("+str(angle)+");", "+":"right("+str(angle)+");","[":"position.append(pos());",
            "]":"pu();goto(position.pop(len(position)-1));","*":"right(180);","<":"pensize(pensize()*2);", 
            ">":"pensize(pensize()/2);","#":"pencolor('blue');","}":"pencolor('red');","&":"pencolor('black');",
            "{":"pencolor('green');","k":"from turtle import*","p":"speed('fastest');","l":"color('black');","m":"position = [];","o":"exitonclick();"}
    for e in newaxiome:
